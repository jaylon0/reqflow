...
| 动作 | 说明 | 必须性 |
|------|------|--------|
| 自检 | 检查本阶段产出的完整性和一致性 | 必须 |
| 问题发现 | 主动识别潜在问题、遗漏、矛盾 | 必须 |
| 自行修复 | 小问题直接修复，记录到报告 | 必须 |
| 辅助 Agent | 按需调用专项 Agent | 按需 |
| 确认点 | 向用户展示结果，等待确认 | 必须 |

**小问题自行修复规则：** 发现的小问题（命名不一致、缺少注释、格式问题等）必须自行修复，但在最终报告中必须列出所有自行修复的内容。
...
**辅助 Agent 输出格式统一为 JSON，包含：** agent_type, status, summary, findings, recommendation

### 多方案对比

技术方案阶段**必须对比 2-3 个候选方案**，输出评估矩阵：
...
全流程中可按需调用辅助 Agent，每个 Agent 有明确的职责边界和触发条件：

```
技术方案阶段 ─┬─ research-agent（技术选型调研）
              └─ architecture-agent（架构方案评估）

代码审查阶段 ─┬─ security-agent（OWASP Top 10 审计）
              └─ performance-agent（性能瓶颈分析）

交付验证阶段 ─── test-gen-agent（补充测试用例）

Agent执行阶段 ─── debug-agent（ReAct 深度调试，修复循环 2 轮未解决时触发）

归档阶段     ─── doc-agent（文档生成和更新）
```

**调用原则：**
- 辅助 Agent 输出为结构化 JSON，主流程解析后决定下一步
- 辅助 Agent 的 findings 纳入最终报告
- 辅助 Agent 的 recommendation 需要用户确认后才执行
...
4. 按阶段顺序执行（必须执行所有阶段）:
   ...
   f. 调用 reqflow_report 报告状态（含自行修复记录）
   g. 如果有门禁，调用 reqflow_verify
   h. 如果门禁未通过，按指引修复
   i. 关键模块完成后暂停等待用户确认
5. 所有阶段完成后，调用 reqflow_dashboard 展示面板
6. 汇总产出物（含所有自行修复记录和辅助 Agent 发现），告知用户等待验收
