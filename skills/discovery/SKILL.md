...
execution_modes:
  lite:  # L0/L1
    ...
    workers: [file-scanner-agent]
    discussion_rounds: 1
  standard:  # L2
    agents: [research-agent, architecture-agent]
    workers: [file-scanner-agent, dependency-agent]
    discussion_rounds: 2
  full:  # L3
    agents: [research-agent, architecture-agent]
    workers: [file-scanner-agent, dependency-agent, code-search-agent]
    discussion_rounds: 2+
---

# discovery

上下文发现阶段。构建代码图、语义索引、影响分析。
...
```
📋 research-agent: 代码图、语义索引、影响分析
```

### 步骤 2: 派遣辅助 Worker

```
🔧 file-scanner-agent: 扫描项目文件结构
...
派遣 Agent 后，⛔ **必须** 使用平台 subagent 能力实际派遣 Agent，然后调用 `reqflow_agent_confirm` 确认完成：
...
| 工具 | 参数 | 结果 |
|------|------|------|
| reqflow_report | stage="上下文发现" | ✅ |
| reqflow_stage_report | stage_name="上下文发现" | ✅ |
...
```
📡 reqflow_report(stage="上下文发现") → ✅ 已记录 (阶段 3/11, 27%)
```
