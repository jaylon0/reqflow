...
execution_modes:
  lite:  # L0/L1
    ...
    discussion_rounds: 1
  standard:  # L2
    agents: [dev-agent, test-gen-agent]
    workers: [code-search-agent, build-agent]
    discussion_rounds: 2
  full:  # L3
    agents: [dev-agent, test-gen-agent, security-agent]
    workers: [code-search-agent, build-agent, test-runner-agent]
    discussion_rounds: 2+
---

# execution

Agent 执行阶段。调度 dev/verify/review Agent，修复循环。
...
| 工具 | 参数 | 结果 |
|------|------|------|
| reqflow_report | stage="Agent执行" | ✅ |
| reqflow_stage_report | stage_name="Agent执行" | ✅ |
| reqflow_dispatch_agent | agent="dev-agent" | ✅ |
...
```
📡 reqflow_report(stage="Agent执行") → ✅ 已记录 (阶段 7/11, 63%)
```
