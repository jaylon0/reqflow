...
**Goal:** Extend `unified_runtime_regression.py` with 6 new test functions covering all 12 integration testing gaps for V2 requirement-flow-plugin.

**Architecture:** Hybrid mode — one full-chain test validates the complete 10-stage artifact chain, plus 5 independent scenario tests cover specific gaps (provider dispatch, repair rounds, multi-item, recovery, adapter factory). All tests use tempfile fixtures for isolation.

**Tech Stack:** Python 3, subprocess, tempfile, runtime_support helpers
...
└── scripts/
    ├── unified_runtime_regression.py    # MODIFY — add helpers + 6 new test functions
    ├── test_adapter_echo.py             # EXISTS — reuse for provider dispatch test
    ├── runtime_support.py               # EXISTS — reuse helpers
    ├── agent_execution_runner.py        # EXISTS — import create_repair_round
    ├── adapter_factory.py               # EXISTS — call via subprocess
    └── workflow_intelligence_runner.py  # EXISTS — trigger multi-layer via keywords
```
...
The existing `main()` function has all logic inline. Before adding new tests, extract reusable helpers.

- [ ] **Step 1: Read the current file**

Read `requirement-flow-plugin/scripts/unified_runtime_regression.py` to understand current structure.

- [ ] **Step 2: Add run_dir fixture helper**

Add after `write_prd()`:

```python
def write_stage_fixtures(run_dir: Path) -> None:
    """Write fixture artifacts for manual/skill stages (01, 05, 06, 08)."""
...
- [ ] **Step 3: Add assert helpers for V2 artifacts**

Add after `assert_text()`:
...
def assert_all_sections_complete(state: dict, sections: list[str]) -> None:
    ...
    for section in sections:
        ...
        assert state[section]["status"] == "complete", f"{section} status is {state[section]['status']}, expected complete"
```

- [ ] **Step 4: Refactor existing main() to use helpers**

Replace the inline setup in `main()` with calls to the new helpers. The existing `main()` function should still work identically:
...
- [ ] **Step 5: Run regression to verify refactor**
...
### Task 2: Add test_full_chain

**Files:**
- Modify: `requirement-flow-plugin/scripts/unified_runtime_regression.py`

- [ ] **Step 1: Add test_full_chain function**

This test runs the complete 10-stage chain including fixture stages 05, 06, 08. Adds assertions for gaps #1, #2, #6, #7, #8.
...
- [ ] **Step 2: Wire test_full_chain into main()**

Add to the test execution block at the bottom. Add a new `ALL_TESTS` list and runner:
...
Run: `cd /Users/yuanjulong/Documents/ai_flow/requirement-flow-plugin && python3 scripts/unified_runtime_regression.py`
Expected: PASS (original main())

Run: `cd /Users/yuanjulong/Documents/ai_flow/requirement-flow-plugin && python3 scripts/unified_runtime_regression.py --test-only`
...
### Task 3: Add test_provider_dispatch_integration

**Files:**
- Modify: `requirement-flow-plugin/scripts/unified_runtime_regression.py`

Note: `delivery_verification_runner.py` does NOT use provider dispatch (hardcodes `missing-provider`). This test verifies the dispatch infrastructure works end-to-end by calling `dispatch_provider()` directly with a configured registry.

- [ ] **Step 1: Add test_provider_dispatch_integration function**
...
### Task 4: Add test_repair_round_loop

**Files:**
- Modify: `requirement-flow-plugin/scripts/unified_runtime_regression.py`

- [ ] **Step 1: Add test_repair_round_loop function**
...
def test_repair_round_loop() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Simulate repair round by importing create_repair_round
        sys.path.insert(0, str(PLUGIN_ROOT / "scripts"))
        from agent_execution_runner import create_repair_round, update_item_status

        # Mark item as failed
        update_item_status(work_items, item["id"], "failed")
        (run_dir / "agent/work_items.json").write_text(
        ...
        assert len(updated_item["execution_record"]["repair_history"]) == 1
        assert updated_item["execution_record"]["repair_history"][0]["round"] == 1

        # Verify repair report exists
        report_path = run_dir / f"agent/reports/{item['id']}/repair-round-1.json"
...
### Task 5: Add test_multi_work_item

**Files:**
- Modify: `requirement-flow-plugin/scripts/unified_runtime_regression.py`

- [ ] **Step 1: Add test_multi_work_item function**

Uses DAO keywords to trigger `java-dao-change` scenario with layers `["dao", "mapper"]`.
...
def test_multi_work_item() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        scenario = assert_json(run_dir / "agent/scenario.json")
        assert scenario["id"] == "java-dao-change"
        assert len(scenario["layers"]) >= 2
...
### Task 6: Add test_recovery_resume

**Files:**
- Modify: `requirement-flow-plugin/scripts/unified_runtime_regression.py`

- [ ] **Step 1: Add test_recovery_resume function**

```python
def test_recovery_resume() -> None:
    """Recovery: run stages 02-03, then resume from stage 04 with existing artifacts."""
    with tempfile.TemporaryDirectory() as tmp:
        tmp_root = Path(tmp)
        project_root, run_dir = setup_run_dir(tmp_root)
        ...
        assert_all_sections_complete(state, [
            "spec_governance", "workflow_intelligence", "context_engine",
            "agent_execution", "delivery_verification", "archive",
        ])
...
### Task 7: Add test_adapter_factory and test_agent_execution_main

**Files:**
- Modify: `requirement-flow-plugin/scripts/unified_runtime_regression.py`

- [ ] **Step 1: Add test_adapter_factory function**
...
def test_adapter_factory() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert output["status"] in ("generated", "success")

        # Verify generated adapter exists
        adapter_path = project_dir / ".dev-workflow.example/adapters/deploy/generic.py"
        assert adapter_path.exists(), f"adapter not found at {adapter_path}"

        # Verify manifest exists
        manifest_path = project_dir / ".dev-workflow.example/adapters/adapter-manifest.yaml"
        assert manifest_path.exists(), "adapter-manifest.yaml not found"

        # Verify generated adapter is executable (returns valid JSON)
        test_input = json.dumps({
            "capability": "deploy", "action": "check",
            "params": {"dry_run": True}, "run_dir": str(project_dir),
        })
        result = subprocess.run(
...
- [ ] **Step 2: Add test_agent_execution_main function**

```python
def test_agent_execution_main() -> None:
    """Agent execution main(): full validation-to-contract flow."""
    with tempfile.TemporaryDirectory() as tmp:
        tmp_root = Path(tmp)
        project_root, run_dir = setup_run_dir(tmp_root)
...
- [ ] **Step 5: Run original regression**

Run: `cd /Users/yuanjulong/Documents/ai_flow/requirement-flow-plugin && python3 scripts/unified_runtime_regression.py`
Expected: PASS (original main() still works)
...
**Files:**
- All regression files

- [ ] **Step 1: Run all regression suites**

Run: `cd /Users/yuanjulong/Documents/ai_flow/requirement-flow-plugin && python3 scripts/unified_runtime_regression.py && python3 scripts/provider_adapter_regression.py && python3 scripts/workflow_intelligence_regression.py && python3 scripts/agent_execution_regression.py`
Expected: All PASS
